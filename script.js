tailwind.config = {
 ...
 function renderRepos(repos, append = false) {
     ...
     if (repos.length === 0) {
         grid.innerHTML = `<div class="col-span-full text-center py-10 text-gray-500 font-bold uppercase tracking-widest text-xs">Sin resultados</div>`;
     ...
     itemsToShow.forEach((repo) => {
         ...
         // Color del lenguaje
         let langColor = '#fff';
         if(repo.language === 'JavaScript') langColor = '#facc15';
         if(repo.language === 'TypeScript') langColor = '#3b82f6';
         if(repo.language === 'Python') langColor = '#22c55e';
         if(repo.language === 'HTML') langColor = '#f97316';
         
         // LÓGICA DEL BOTÓN WEB (ARREGLADA)
         const hasWeb = repo.homepage && repo.homepage.trim() !== "";
         ...
         card.innerHTML = `
             <div class="flex justify-between items-start mb-4">
                 ...
                 <div class="flex gap-2">
                     ${hasWeb ? `
                     <a href="${repo.homepage}" target="_blank" onclick="event.stopPropagation()" class="flex items-center gap-1 px-2 py-1 bg-primary/10 border border-primary/50 text-primary rounded-md text-[10px] font-bold uppercase hover:bg-primary hover:text-black transition-all" title="Ver Web en Vivo">
                         <i data-lucide="globe" class="w-3 h-3"></i> WEB
                     </a>` : ''}
                     
                     <a href="${repo.html_url}" target="_blank" onclick="event.stopPropagation()" class="p-1.5 bg-white/5 hover:bg-white/20 rounded-md text-gray-400 hover:text-white transition-colors">
             ...
             <p class="text-xs text-gray-400 mb-6 flex-1 line-clamp-2 leading-relaxed">${repo.description || 'Sin descripción.'}</p>
             ...
             <div class="flex items-center justify-between text-[10px] font-mono uppercase font-bold text-gray-500 pt-3 border-t border-white/5 w-full">
                 <div class="flex items-center gap-2">
                     ${repo.language ? `<span class="w-2 h-2 rounded-full" style="background-color: ${langColor}"></span> ${repo.language}` : ''}
                 </div>
         ...
         `;
         card.onclick = () => openRepoViewer(repo);
 ...
 function filterByLang(lang, btnElement) {
     ...
     document.querySelectorAll('#filter-container button').forEach(b => {
         b.className = 'bg-black/60 hover:bg-white/20 text-gray-300 px-4 py-2 rounded-lg text-[10px] font-bold uppercase tracking-wider whitespace-nowrap transition-all border border-white/20';
     });

     if (currentLangFilter !== 'all') {
         btnElement.className = 'bg-white text-black px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-wider whitespace-nowrap border border-white scale-105';
     } else {
 ...
 async function openRepoViewer(repo) {
     ...
     fileTree.innerHTML = '<div class="text-center p-4 text-primary animate-pulse font-mono text-[10px] uppercase font-bold">Cargando...</div>';
     ...
     try {
         ...
         if (data.tree) {
             const sorted = data.tree.filter(i => i.type === 'blob').sort((a, b) => a.path.localeCompare(b.path));
             renderFileTree(sorted, repo.name, repo.default_branch);
         }
     } catch (e) {
         fileTree.innerHTML = '<div class="text-red-400 p-2 text-center text-[10px]">Error API</div>';
     }
 }
 ...
 function renderFileTree(files, repoName, branch) {
     ...
     files.forEach(file => {
         ...
         div.className = 'flex items-center gap-2 p-2 hover:bg-white/5 rounded cursor-pointer text-[10px] text-gray-400 hover:text-white transition-colors truncate font-mono';
         div.innerHTML = `<i data-lucide="file" class="w-3 h-3"></i> ${file.path}`;
 ...
 async function loadFileContent(repoName, branch, path, element) {
     ...
     viewer.innerHTML = '<p class="text-center text-gray-500 font-mono text-xs">Cargando...</p>';

     try {
         const res = await fetch(`https://api.github.com/repos/${USERNAME}/${repoName}/contents/${path}?ref=${branch}`);
         const data = await res.json();
         const content = atob(data.content);
         viewer.innerHTML = `<pre class="font-mono text-[10px] text-gray-200 whitespace-pre-wrap break-words">${content.replace(/[&<>"']/g, m => ({'&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;','\'':'&#039;'}[m]))}</pre>`;
     } catch (e) {
         viewer.innerHTML = '<p class="text-center text-red-400 font-mono text-xs">Error de lectura</p>';
     }
 }
